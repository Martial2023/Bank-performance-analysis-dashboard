...
import { getDominantAccountCount } from '@/app/actions';
import { DominantAccountCountProps } from '@/lib/types';
import React, { useEffect, useState } from 'react';
import { BarChart, Bar, XAxis, YAxis, Tooltip, ResponsiveContainer } from 'recharts';
import { Button } from '../ui/button';
import TopAccountCredenzaChart from './TopAccountCredenzaChart';

const TopAccountChart = () => {
    const [isLoading, setIsLoading] = useState<boolean>(true);
    const [dominantDistrictCount, setDominantDistrictCount] = useState<DominantAccountCountProps[]>([]);
    ...
    const fetchDominantDistrictCount = async () => {
        try {
            setIsLoading(true);
            setError(null);
            const data = await getDominantAccountCount();
            if (data) {
    ...
    return (
        <div className="w-full h-full flex flex-col p-2">
            <div className="px-4 pt-2 pb-4">
                <h2 className="text-lg font-medium text-gray-800 dark:text-gray-200">Dynamisme des comptes</h2>
                <p className="text-sm text-gray-500 dark:text-gray-400 text-center">Top 10 des Clients</p>
            </div>
...
export default TopAccountChart;